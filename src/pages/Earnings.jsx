...
export default function Earnings() {
  ...
  if (error || !data)
    return <div className="p-6 text-danger">{error || "No data"}</div>;
  ...
  return (
    <section className="min-h-full bg-background p-4 sm:p-6 pb-20">
      ...
      {/* Filters Top Bar */}
      <Card noPadding className="mb-6 flex flex-col">
        <FilterPanel
          search={
          ...
          actions={
            <Button
              variant="secondary"
              className="gap-2 bg-surface text-foreground font-medium px-4 h-10 shadow-sm border border-border shrink-0 w-full sm:w-auto"
              onClick={() => exportToCSV(topPartners, "earnings.csv")}
            >
        ...
        <Card className="p-5 xl:col-span-2 flex flex-col h-[400px]">
          ...
          <div className="flex gap-6 mb-4 justify-center text-sm font-semibold">
            <span className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-primary"></span> Total
              Earnings
            </span>
            <span className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-success"></span> Partner
              Earnings
            </span>
            <span className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-info"></span> Commission
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 flex flex-col justify-between">
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-surface text-muted font-medium border-b border-border">
                  <tr>
                    ...
                    <th className="px-5 py-4 font-medium text-right text-foreground">
                      This Period (12 May - 20 May)
                    </th>
                    <th className="px-5 py-4 font-medium text-right text-foreground">
                      Previous Period (04 May - 11 May)
                    </th>
                    <th className="px-5 py-4 font-medium text-right text-foreground">
                      Change %
                    </th>
                ...
                <tbody className="divide-y divide-border font-semibold">
                  {/* Partner Earnings */}
                  <tr>
                    <td className="px-5 py-4 text-foreground font-normal">
                      Delivery Partner Earnings
                    </td>
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.partner.current)}
                    </td>
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.partner.previous)}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3 text-success">
                        <span className="text-xs">
                          {
                            formatDelta(
                              summary.partner.current,
                              summary.partner.previous,
                            ).diff
                          }
                        </span>
                  ...
                  <tr>
                    ...
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.platform.current)}
                    </td>
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.platform.previous)}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3 text-success">
                        <span className="text-xs">
                          {
                            formatDelta(
                              summary.platform.current,
                              summary.platform.previous,
                            ).diff
                          }
                        </span>
                        <span className="text-xs font-semibold">
                  ...
                  <tr>
                    ...
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.other.current)}
                    </td>
                    <td className="px-5 py-4 text-right text-foreground font-normal">
                      {formatAmount(summary.other.previous)}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3 text-success">
                        <span className="text-xs">
                          {
                            formatDelta(
                              summary.other.current,
                              summary.other.previous,
                            ).diff
                          }
                        </span>
                        <span className="text-xs font-semibold">
                  ...
                  <tr>
                    ...
                    <td className="px-5 py-4 text-right text-danger font-normal">
                      -{formatAmount(summary.refunds.current)}
                    </td>
                    <td className="px-5 py-4 text-right text-danger font-normal">
                      -{formatAmount(summary.refunds.previous)}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3 text-danger">
                        ...
                        <span className="text-xs">
                          +
                          {formatAmount(
                            summary.refunds.current - summary.refunds.previous,
                          )}
                        </span>
                        <span className="text-xs font-semibold flex items-center gap-0.5">
                          <TrendingDown size={14} />{" "}
                          {(
                            ((summary.refunds.current -
                              summary.refunds.previous) /
                              summary.refunds.previous) *
                            100
                          ).toFixed(2)}
                          %
                ...
                <tfoot>
                  <tr className="bg-primary/5 border-t-2 border-border font-bold">
                    <td className="px-5 py-4 text-foreground text-base">
                      Total Earnings
                    </td>
                    <td className="px-5 py-4 text-right text-foreground text-base">
                      {formatAmount(stats.total)}
                    </td>
                    <td className="px-5 py-4 text-right text-foreground text-base">
                      {formatAmount(1021538)}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3 text-success">
                        <span className="text-xs">
                          +{formatAmount(stats.total - 1021538)}
                        </span>
          ...
          <div className="text-xs text-muted flex items-center gap-1.5 mt-6 xl:mb-4">
            <Info size={14} /> Earnings are updated every 24 hours. Last updated
            on{" "}
            <span className="font-semibold text-foreground">
              20 May 2024, 11:30 PM
            </span>
          </div>
        </div>
        ...
        <Card className="p-0 overflow-hidden flex flex-col">
          <div className="p-5 border-b border-border flex justify-between items-center bg-surface">
            <h3 className="font-bold text-foreground">
              Top Earning Delivery Partners
            </h3>
            <Link
              to="/delivery"
              className="text-primary text-sm hover:underline pr-2"
            >
              View All
            </Link>
          </div>
            ...
            >
              {paginatedPartners.map((dp, index) => (
                <tr key={index}>
                  ...
                  <td className="px-5 py-4 text-center font-semibold text-muted">
                    {dp.orders}
                  </td>
                  <td className="px-5 py-4 text-right font-bold text-foreground">
                    {formatAmount(dp.earnings)}
                  </td>
                </tr>
              ))}
            </Table>
